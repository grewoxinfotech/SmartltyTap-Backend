...
const { apiControlMiddleware } = require("./middleware/apiControl");
...
function createApp() {
  ...
  const allowedOrigins = [env.appBaseUrl, "http://localhost:3000", "http://localhost:3001", "http://localhost:5000"];
  app.use(
    cors({
      origin: (origin, cb) => {
