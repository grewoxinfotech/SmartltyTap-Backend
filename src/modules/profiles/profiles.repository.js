  ...
  const [prof] = await Profile.upsert(
    {
      ...
      brochure_name: data.brochure_name,
    },
    { returning: true }
  );
  return prof;
...
async function listProfiles(limit = 100, offset = 0) {
  const profiles = await Profile.findAll({ limit, offset, order: [['id', 'DESC']] });
  return profiles;
}

module.exports = { upsertProfile, setLink, getProfile, listProfiles };
